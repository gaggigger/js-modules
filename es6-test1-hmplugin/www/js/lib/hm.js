...
(function () {
    ...
    function cleanModuleName(text) {
        var moduleName = moduleNameRegExp.exec(text)[1],
            ...
            needsPrefix = !(requirejs.s.contexts._.config.shim && requirejs.s.contexts._.config.shim[moduleName] || moduleName == 'jquery');

        if (index === -1 && needsPrefix) {
            // Needs the hm prefix.
    ...
    function expandImportRefs(moduleMap, text, moduleName) {
        ...
        if (parts[0] === '*') {
            ...
            for (i = 0; (part = parts[i]); i++) {
                ...
                // Check it's not a jquery plugin without module definition or anything also without module definition

                var strippedName = moduleName.split("'").join(""); // convert 'moduleName' to moduleName
                var addColon = !(requirejs.s.contexts._.config.shim && requirejs.s.contexts._.config.shim[strippedName] || strippedName == 'jquery');

                if(addColon){
                    modifiedText += '.' + colonParts[0];
                }
                ...
                if (type === 'module') {
                    ...
                    if (fromIndex > 0) {
                        ...
                        for (j = fromIndex - 1; j >= 0; j--) {
                            ...
                            //var nextChar =  spaceParts[j].charAt(0)
    ...
    function compile(text, config, moduleName) {
        var stars = [],
            moduleMap = {},
            transforms = {},
            currentIndex = 0,
        ...
        // VERY DIRTY HACK! transform System.import().then() to require([], function).
        transformedText = transformedText.replace('System.import(\'', 'require([\'hm!');
        transformedText = transformedText.replace(').then(', '], ');
    ...
    define({
        ...
        load: function (name, require, load, config) {
            var path = require.toUrl(name + '.js');
            fetchText(path, function (text) {
                var result = compile(text, config.hm, name);
